...
3. **添加到场景** → 在场景画布上**拖拽移动、拖右下角黄手柄缩放、点击选中**（图层栏自动定位展开对应卡片）；`Ctrl/Cmd + 点击`可多选，`Ctrl/Cmd + G`建立父级组，`Ctrl/Cmd + Shift + G`解组。
...
11. **统一裁剪蒙版**：任意 Runtime 图层（图片、图元、粒子、builtin、兼容项目元素）可使用矩形、椭圆或多边形 `mask`，预览、独立 Runtime、PNG 与 GIF 共用 Canvas `clip()` 渲染。卡片只保留类型下拉与一个动态“框选 / 编辑 / 完成”按钮；编辑时画布显示绿色轮廓、控点与外部暗化，内部保持可见。矩形/椭圆可拖内部移动、拖绿角缩放，且椭圆始终以真实椭圆预览；多边形单击加点、双击或 Enter 完成、拖绿色点调整、点边线插点，Esc 取消、Delete 删除选中顶点。粒子卡片原“发射区”更名为“发射范围”：它只决定粒子出生位置；裁剪蒙版决定最终可见范围。两者共用画布框选，但保持不同语义。
    ...
    多选联合框可批量移动、旋转、缩放、镜像、复制或删除；图片、矢量与带稳定 id 的程序 parts 元素共用选择/编组路径。建立父级组后联合框显示为紫色，组卡片收纳成员；Canvas 点成员会展开并定位其卡片。组同步优先采用唯一非空动画/滚动值，否则取组内最上方成员。父级组不增加渲染层，组动画以逐成员共享矩阵实现；单击空白处或 Esc 退出选择，双击成员进入其 parts 编辑。元素卡片还可对最终 Alpha 轮廓添加硬边阴影与 1–4px 外描边。
