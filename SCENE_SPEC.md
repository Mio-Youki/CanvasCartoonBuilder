...
- 元素可选样式包含 `style.samplingMotion:{show?,field?,edge?,pixelBoil?,thresholdPulse?,ditherDrift?,grainTide?}`。四个子效果是独立开关并允许叠加，固定顺序为“像素沸腾 → 颗粒潮汐局部采样 → 阈值呼吸 → 网点游移”；`show` 属于能力组，统一限制作用时段。`field:{type:'uniform'|'wave'|'radial',scale,speed,angle?,centerX?,centerY?,period,steps,seed}` 是共享空间控制器；`edge:{mode:'alpha',width,strength}` 抑制透明轮廓附近的采样运动，完全不透明图片只能保护外框。`pixelBoil:{rate,amount,mode:'step'|'drift',seed}` 要求 `pixelDiv>1`；`thresholdPulse:{threshold,amount,period,steps,dark,light}`；`ditherDrift:{cell,period,steps,direction:'horizontal'|'vertical'|'diagonal',dark,light}`；`grainTide:{amount,coarseness,targets:'combined'|'sampling'|'threshold'|'dots'}` 使用共享场让局部颗粒成片运动。“轮廓稳定、内部翻涌”只是编辑器一次写入 `field + edge + grainTide` 的组合预设，不新增 Runtime 类型。`steps` 为 2..60 的缓存时间步；相同 `t + seed` 必须确定，周期首尾闭合。首版 `grainTide` 内的波场参数及 `style.sampleJitter`、`imageFx.motion` 仍由 Runtime 兼容读取，编辑器会迁移到新结构。其它元素样式仍为 `style:{shadow?:{color,distance,angle},outline?:{color,width},imageFx?:{mode:'threshold'|'halftone',threshold?,dark?,light?,cell?},bandWave?:{amp?,period?,band?,cycles?}}`。阴影无模糊；静态 `imageFx` 不能替代语义分层；`bandWave` 只做水平带状位移。图片与 parts 都可设置 `pixelDiv`（≥1，无 UI 人工上限）和 `alphaMode:'keep'|'remove'|'boost'`。效果基于图片与 parts 合成后的 Alpha 轮廓，在元素自身蒙版内合成；元素不可见时不会绘制。
